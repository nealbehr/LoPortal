<?php
namespace LO\Provider;

use LO\Validator\UniqueValidator;
use Silex\Application;
use Silex\ServiceProviderInterface;


class UniqueValidatorServiceProvider implements ServiceProviderInterface {

    /**
     * Registers services on the given app.
     *
     * This method should only be used to configure services and parameters.
     * It should not get services.
     *
     * @param Application $app An Application instance
     */
    public function register(Application $app){
        if(!isset($app['validator.validator_service_ids'])){
            $app['validator.validator_service_ids'] = [];
        }

        $app['validator.validator_service_ids'] = array_merge((array)$app['validator.validator_service_ids'], ['validator.unique' => 'validator.unique']);

        $app['validator.unique'] = $app->share(function ($app) {
            $validator =  new UniqueValidator($app['orm.em']);

            return $validator;
        });
    }

    /**
     * Bootstraps the application.
     *
     * This method is called after all services are registered
     * and should be used for "dynamic" configuration (whenever
     * a service must be requested).
     */
    public function boot(Application $app)
    {
        // TODO: Implement boot() method.
    }
}